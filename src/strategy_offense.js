...
class Strategy {
  ...
  // ---------- 主动进攻(核心:攒够兵就往敌方腹地打,找将斩首) ----------

  /**
   * 攒出一支足够大的兵团后,不再原地干等,而是压进敌方领土、直扑其"腹地"
   * (可见敌格的重心 ≈ 敌将附近)。一路吃敌格、获得视野,迟早撞见敌将,
   * 届时 tryStrike(优先级更高)接管一波带走。这是之前完全缺失的一环——
   * 老版本只会铺地聚兵,从不进攻,所以永远找不到、也打不掉敌将。
   */
  tryOffense() {
    const gs = this.gs;
    const OFFENSE_MIN = +process.env.OFF_MIN || 30; // 兵团达到此规模才发起进攻

    const enemy = [];
    for (let t = 0; t < gs.size; t++) if (gs.isEnemy(t)) enemy.push(t);
    if (!enemy.length) return null; // 还没接触到敌人,先扩张

    const src = this.biggestArmyTile([]);
    if (src === -1 || gs.armies[src] < OFFENSE_MIN) return null;

    // 目标:可见敌格的重心(敌方领土大致从将军辐射开,重心通常离敌将不远)
    let sr = 0, sc = 0;
    for (const t of enemy) { sr += gs.row(t); sc += gs.col(t); }
    sr /= enemy.length; sc /= enemy.length;
    let target = -1, bd = Infinity;
    for (const t of enemy) {
      const d = Math.abs(gs.row(t) - sr) + Math.abs(gs.col(t) - sc);
    ...
    return this.planPath(buildPath(prev, target), 'offense');
  }
