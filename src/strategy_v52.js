...
class Strategy {
  ...
  tryDenyVision() {
    // DENY_OFF=1:整个行为关掉,用来先量"这个行为值多少",再决定值不值得修它
    // 36.6% 的完成率(计划 5.47 步只走 2 步)。
    if (P('DENY_OFF', 0)) return null;
   ...
   */
  /**
   * 【v52】允许"过渡步":踩自己的地块继续走。
   *
   * exec_audit.js 实测 v51:raid 出发兵力中位 15,只走 2 步、吃 2 块地,
   * **手里还剩 7 兵(50%)花不掉**;带兵>5 的 raid 有 60.7% 花不掉一半。
   * 拆开停下来的 199 次,末端未访问邻居的构成:
   *   真死路(山/城/已访问/迷雾) 50.3%,只被自己的地挡住 27.1%,吃不起的敌兵 11.1%。
   *
   * 原实现的候选只有"敌格 + 已知空地",三条约束叠加把蛇走进死角:
   *   (1) 不许踩自己的地;(2) visited 一律不许回头 —— 可蛇走过的格子会变成我的地,
   *   回头踩是合法的;(3) 迷雾不走。
   *
   * 兵力账(与开局规划器同一条,见 current-state-and-next-step 记忆):
   * 从 A 兵的格子走到自己的 1 兵地 → (A-1)+1 = A,**过渡不花兵,只花时间**。
   * 所以过渡不该受兵力预算限制,只该受时间限制 —— 用 RAID_HOP 限制**连续**过渡步数,
   * 防止蛇在自家领地里瞎逛(开局规划器第四次尝试就是栽在"多出来的步数从别处偷来"）。
   */
  raidWalk(src) {
    const gs = this.gs;
    const P_ = (k, d) => (process.env[k] !== undefined ? +process.env[k] : d);
    const MAX_HOP = P_('RAID_HOP', 2);   // 连续过渡步上限;0 = 完全退回 v51 行为
    const path = [src];
    const visited = new Set([src]);
    let strength = gs.armies[src] - 1;
    let cur = src;
    let hop = 0;                          // 已连续走了几个过渡步
    while (strength > 1) {
      let pick = -1, pickScore = -Infinity, pickKind = null;
      for (const n of gs.neighbors(cur)) {
        if (visited.has(n)) continue;
        if (gs.isCity(n)) continue;
        const enemyTile = gs.isEnemy(n);
        const emptyTile = gs.terrain[n] === -1;
        // 过渡步:自己的地。不花兵(栈原样带走),但占一个半回合。
        const ownTile = gs.isMine(n);
        if (!enemyTile && !emptyTile && !ownTile) continue;
        if (ownTile && hop >= MAX_HOP) continue;
        const cost = enemyTile ? gs.armies[n] + 1 : (emptyTile ? 1 : 0);
        if (strength - cost < 1) continue;
        // 吃敌地最优(我+1他-1),其次占空地,过渡步最次 —— 只在没得吃时才用。
        const score = (enemyTile ? 100 : emptyTile ? 0 : -50) - cost;
        if (score > pickScore) { pickScore = score; pick = n; pickKind = enemyTile ? 'e' : emptyTile ? 'm' : 'o'; }
      }
      if (pick === -1) break;
      strength -= pickKind === 'e' ? gs.armies[pick] + 1 : (pickKind === 'm' ? 1 : 0);
      hop = pickKind === 'o' ? hop + 1 : 0;   // 吃到东西就把过渡预算清零
      path.push(pick);
      visited.add(pick);
      cur = pick;
    }
  ...
  tryCaptureCity() {
    ...
    const CITY_SIDE = P('CITY_SIDE', 0); // 允许比对手近多少格的松弛量
    const cityDefensible = (c) => {
      if (gs.isEnemy(c)) return true; // 打对手的塔是削他产能,另一套账
      ...
      return gs.dist(c, gen) <= dEnemy + CITY_SIDE;
    };

    const eligible = (c) => (gs.isEnemy(c) ? canEnemyCity : (canNeutralCity && cityDefensible(c)));
