...
class Strategy {
  ...
  tryExpand() {
    const gs = this.gs;
    const earlyGame = gs.turn < 50;
    const cheapOnly = !earlyGame;
    if (earlyGame) {
      const src = this.biggestArmyTile([]);
      if (src !== -1 && gs.armies[src] > 1) {
        const path = this.expansionWalk(src);
        if (path.length >= 2) return this.planPath(path, 'expand');
      }
    }
    let best = null;
    for (let t = 0; t < gs.size; t++) {
    ...
    for (let t = 0; t < gs.size; t++) {
      if (gs.terrain[t] === -1 && !gs.isCity(t) && dist[t] < bd) { bd = dist[t]; target = t; }
    }
