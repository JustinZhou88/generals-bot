...
class Strategy {
  ...
  nextMove() {
    ...
    // 【v56】兵堆"陈旧度"追踪:记录每个己方格子上次**出过兵**的时刻。
    //
    // stranded_audit.js 实测 v54:**24.4% 的可动兵力处于滞留状态**
    // (>=15 兵、连续 30 个半回合没出过兵),平均每时点 66.3 兵、2.37 个兵堆。
    // 机制在 gatherToward 的兵源打分 eff = (兵-1)/(距离+1) —— 距离惩罚极重,
    // 距离 15 的 30 兵堆(eff 1.8)永远输给距离 1 的 5 兵堆(eff 2.0),
    // 于是远处的大兵堆被**永久饿死**。这里给它一个随时间增长的权重。
    // STALE_W=0 时不追踪也不加权,完全等价 v54。
    if (P('STALE_W', 0)) {
      if (!this._staleSince) { this._prevArmy = new Map(); this._staleSince = new Map(); }
      for (let t = 0; t < gs.size; t++) {
        if (!gs.isMine(t)) { this._prevArmy.delete(t); this._staleSince.delete(t); continue; }
        const a = gs.armies[t];
        const pa = this._prevArmy.get(t);
        // 兵力减少 = 从这里出过兵 → 计时清零;新占的地也从现在算起
        if (pa === undefined || a < pa) this._staleSince.set(t, gs.turn);
        this._prevArmy.set(t, a);
      }
    }
    ...
    if (mm && mm.pause && P('PAUSE_GATHER', 1)) {
  ...
  gatherToward(target) {
    ...
    for (let t = 0; t < gs.size; t++) {
      ...
      if (dist[t] === Infinity) continue;
      let eff = (gs.armies[t] - 1) / (dist[t] + 1);             // 每步收兵数
      // 【v56】陈旧度加权:越久没动过的兵堆权重越高,避免远处大兵堆被永久饿死。
      const SW = P('STALE_W', 0);
      if (SW && this._staleSince) {
        const since = this._staleSince.get(t);
        const idle = since === undefined ? 0 : gs.turn - since;
        eff *= 1 + SW * Math.min(idle, P('STALE_CAP', 120)) / 100;
      }
