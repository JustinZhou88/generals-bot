 ...
 * 模仿学习排序器策略 v2(线上推理端)。
 *
 * 【v4】模型加载(全量语料重训):优先 replays/imitation/model3.json(MLP,32 维,含"不动"
...
// 【v4】只用**精英玩家**(asdvlaksudhiouwenf 68%胜率/对手73.4星、where are you? 63.7%、
// YiShengYiShi 83.1%)的 632 局训练。排除了 zhuxueliang(43.3%胜率)和 -zzh-(46.2%)——
// 语料从 469 局扩到 1845 局时加进来的正是这两个弱玩家,这解释了为什么
// 数据翻四倍反而更差(v43 46.3%):风格冲突的专家平均起来比任何单个专家都差。
// 假设:语料从 469 局精选扩到 1845 局时混入了大量不同水平的玩家(含输家),
// 模型学会了模仿平庸走子 —— 全量模型准确率更高(65.84%)却打得更差(46.3%)。
// 单独一个模块而不是覆盖 model3.json —— 否则会同时改掉已发布的 v28/v29/v42 的行为,
// A/B 就不公平了。
const MODEL4_PATH = path.join(IMIT_DIR, 'model_elite.json');
const MODEL3_PATH = path.join(IMIT_DIR, 'model3.json');
const MODEL = fs.existsSync(MODEL4_PATH) ? require(MODEL4_PATH) : require(MODEL3_PATH);

const IS_MLP = MODEL.type === 'mlp';
const FEAT = MODEL.feat || (MODEL.w && MODEL.w.length) || 32;
const V2 = FEAT >= 32;

// 线性通路:MLP 用 wlin,老模型用 w
const W_VEC = IS_MLP ? MODEL.wlin : MODEL.w;
const BIAS = MODEL.b || 0;
const H = IS_MLP ? MODEL.H : 0;
const W1 = IS_MLP ? Float64Array.from(MODEL.W1) : null;
const B1 = IS_MLP ? Float64Array.from(MODEL.b1) : null;
const VOUT = IS_MLP ? Float64Array.from(MODEL.v) : null;

/**
 * 打分器。必须与 train3.js 的 scoreOne 逐位一致,否则线下准确率不代表线上行为。
 *   线性:  score = b + w·x
 *   MLP:   score = b + wlin·x + v·tanh(W1 x + b1)
 */
function scoreVec(x) {
  let s = BIAS;
  ...
  for (let h = 0; h < H; h++) {
    ...
    s += VOUT[h] * Math.tanh(z);
  }
  return s;
}

// "不动"伪候选的特征向量:f25=1,其余全 0。得分是常数,启动时算一次。
const PAUSE_X = new Array(FEAT).fill(0);
if (FEAT >= 25) PAUSE_X[24] = 1;
const PAUSE_BASE = scoreVec(PAUSE_X);
...
class ImitationStrategy {
  ...
  nextMove() {
    ...
    if (V2 && !process.env.PAUSE_OFF) {
      ...
      const pauseScore = PAUSE_BASE + (process.env.PAUSE_BIAS !== undefined ? +process.env.PAUSE_BIAS : 0);
