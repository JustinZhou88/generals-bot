...
/**
 * strategy_v55.js = v51 + 唯一一处:"兵不够别撞塔"的保护闩
 *
 * 【为什么是这个组合】2026-07-30。v54 曾把两项改动一起发布,但其中
 * `PAUSE_GATHER`(空转期免费调兵)在真人实战里被证伪:
 *   21 局 v54 实战,对 **≥20★ 对手 0 胜 14 负**(v51 同时段同分段是 8-6 = 57.1%);
 *   若真实胜率 57.1%,连输 14 局概率 ~1.1e-5;局长中位 166 → 260。
 *   而三个离线裁判(siege +5.4pp / corpus +4.25pp / 直接对位 55.4% 显著)**全部给了正号**。
 * 所以 v55 只保留另一项 —— 它是纯保护闩,机制上不可能致负。
 *
 * 【v55 相对 v51 的实测】(全部多种子合并)
 *   siege_test(主裁判,3 种子 1800 局)  64.5% → **66.8%**(+2.3pp,三种子同号)
 *   corpus_arena(1600 局)              63.00% → 63.81%
 *   直接对位 v51(4 种子 1800 局)        51.9% [49.6, 54.2] —— 平局(不弱)
 *   全代际 32 前代 × 2 种子             无任何前代显著强于它
 * 结论:**不显著更强,但也不弱,且修掉了一个实测每 79 局白扔 516 兵的漏洞。**
 *
 * 【本文件刻意不含的东西】以下都测过且被否,不要再加回来:
 *   PAUSE_GATHER(真人实战 ≥20★ 0-14)、CITY_SIDE_E(两裁判无收益)、
 *   HOME_PEN(相关性 r=-0.665 是实的,但这个实现在 siege 上不转化)、
 *   DEFEND_YIELD(直接对位 49.0%)、STALE_W(49.6%)、
 *   CITY_STEP(修了一个真 bug 却 corpus −4pp,那个 bug 是承重的)。
 *
 * ---- 以下是继承自 v51 的说明 ----
 *
 * strategy_v51.js = v45 + 按对手类型切换侦察策略(bot 用封顶/真人瞄先验深度)
  ...
  popValidQueued() {
    ...
    while (this.queue.length) {
      ...
      if (gs.isMine(mv.from) && gs.armies[mv.from] > 1 && gs.isPassable(mv.to)) {
        ...
        // **败局里的频率是胜局的近 4 倍**(0.35 vs 0.09 次/局)。
        //
        // 修在校验层而不是各个 try* 里,这样所有来源(strike/city/gather/模型走子)都兜得住。
        // CITY_BOUNCE=1 可关掉这道闩(退回 v51 行为),仅用于 A/B 对照。
        if (!P('CITY_BOUNCE', 0) && gs.isCity(mv.to) && !gs.isMine(mv.to) &&
