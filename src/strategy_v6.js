...
class Strategy {
  ...
  nextMove() {
    ...
    // 一旦敌人摸到将军附近(老家已暴露),就别再盲目铺地——锁存这个状态,
    // 之后把精力转向防守/找将反打,而不是继续跑去远处种田。
    if (!this.homeExposed) {
      const gen = gs.myGeneral();
      for (let t = 0; t < gs.size; t++) {
        if (gs.isEnemy(t) && gs.dist(t, gen) <= 4) { this.homeExposed = true; break; }
      }
    }
    ...
    // 开局按兵不动到第 13 回合(=26 半回合),让将军先攒到 ~14 兵,再一波长蛇甩出去、
    // 触角伸得更远,后期不用回家踩着自己的地远程调兵。防守已在上面处理,故不怕早鲨。
    if (gs.turn < 26 && !this.homeExposed) return null;
