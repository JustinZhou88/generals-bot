...
/**
 * strategy_v51.js(候选)= v45 + 按对手类型切换侦察策略(bot 用封顶/真人瞄先验深度)

 * 唯一改动:tryScout 的 projCap = 已见敌格最深投影 × SCOUT_CAP,从 1.3 收到 1.05。
 * 含义是"别越过对手的前沿往他身后的空地扎" —— 这和六次机制改动得出的结论一致:
 * 找将唯一有效的信号是**方向**(敌将在对手领地后方),而 v29 原来的 1.3 冲过头了。
 * 注意 0.8 会直接崩到 28.7%(够不着对手前沿),1.0~1.05 是悬崖边的最优点。
 *
 * 找将率(三个种子合并 900 局):52.7% → 61.4%,+8.7pp。
 * 按作弊实验测出的转化系数(每 +10pp 找将 ≈ +6pp 胜率),预期约 +5pp。

 * 来自 2026-07-27 与用户的真人对局(protodump_live 全程录制):
 *   [1] 打塔要"守得住"才打。实测:ht146 打下一座距自家将军 14 格、
 *       距对手地块仅 1 格的中立塔,砸进 40 兵后驻军只剩 15,
 *       13 个半回合后被对手 56 兵团收走 —— 一来一回近 80 兵的净亏,
 *       兵力从 150:157 持平直接崩成 174:242,整局由此翻盘。
 *       用户原话:"刚打塔就被我发现然后被我占领,这在开局不是好事"。
 *   [2] 侦察不该往地图边角冲。原实现是"沿(我将军→敌重心)方向投影最大的格",
 *       无上限 ⇒ 直接越过对手扎进其身后的空地角落。实测第二局 328 个半回合
 *       全程没找到敌将(斩首整局未触发),主力被侦察反复拉去南边空地。
 *       用户原话:"为什么第二局会莫名其妙跑到下面去?下面有没有对手"。
 *
 * strategy_v28.js — 基座 v19 + 模仿学习 v3(MLP)混合体
 *
 * 为什么基座是 v19 而不是 v27:用与官方逐帧一致的模拟器(见 conformance.js)
 * + 真分布随机地图(mapgen.js)跑完 27 版全代际大循环赛(351 组 × 24 局)后,
 * 真实排名是 v19 61.7% > v18 > v17 > v16 > v15 > v21 … > v24 51.0% > v25 49.2%
 * … > v26 31.9% > v27 32.4%。也就是说 v20 之后这条线一直在退步,只是旧 arena
 * (固定 28 张地图、每次 6 局)样本太小、地图太少,完全看不出来。
 * v28 因此回到 v19 这个真正的最强基座上做加法。
 *
 * 加的这一件事:v24 验证过的"混合体"——规则握着所有生死逻辑(开局波次/斩首/
 * 防守/反蹲/攻城/节奏),只把发育段的"日常行军"交给模仿学习排序器。
 * 排序器换成 v3 的 MLP(留出集 top1 54.1% → 57.5%),走 src/imitation3.js。
 */
...
class Strategy {
  ...
  nextMove() {
    ...
    // 老家是否暴露:够分量的敌兵(≥6)最近 30 个半回合内出现在将军 4 格内。
    // 1 兵的"眼"不算兵临城下——它锁不住我们的进攻,只该被立刻清掉(见 tryDenyVision)。
    {
      const gen = gs.myGeneral();
      for (let t = 0; t < gs.size; t++) {
        if (gs.isEnemy(t) && gs.armies[t] >= 6 && gs.dist(t, gen) <= 4) { this.lastExposedTurn = gs.turn; break; }
      }
    ...
    if (gs.turn < P('OPEN_TURN', 24) && !this.homeExposed) return null;
    ...
    if (gs.turn < 50) {
      ...
      if (wave) return wave;
      // 波间是否真的该按兵不动:实测第25回合仍有 ~25 兵没花出去、50 个半回合空转 26.8 个。
      // OPEN_FILL=1 时改为落回常规扩张,把闲置兵换成地。
      if (this.waveCount < P('WAVE_MAX', 4)) {
        if (!P('OPEN_FILL', 0)) return null;
        const fill = this.tryExpand();
        if (fill) return fill;
        return null;
      }
    }
    ...
    if (gs.turn >= 70 && gs.turn < 100) {
      ...
      return this.tryStrike() || this.tryExpand() || this.tryScout() || this.tryGather();
    }
    ...
    // 践踏窗口:用户本人的打法是"距离翻倍还剩 10 turn 左右就出去踩对手的地"。
    // v19 原值是 34(=翻倍前 8 个真实回合),RAID_PHASE 可调到 30(=10 回合)。
    const raidWindow = phase >= P('RAID_PHASE', 34) && gs.turn >= P('RAID_FROM', 100);
    ...
    // 发育段(turn>=100):生死逻辑仍由规则决定,日常行军交给模仿学习排序器。
    const high = (
      this.tryStrike() ||
      this.tryCounterCamp() ||
    ...
    // 【v54】模型说"pause"时,原来直接 return null —— 把后面整条
    // tryExpand/tryHarass/tryGather 全跳过,一步不走。
    //
    // idle_audit.js 实测(40 局):ht100~200 空转 6.8%、ht200~400 空转 4.9%,
    // 而**空转当下平均有 127.4 兵散在 67.7 个格子上**,离最近敌格才 4.2 格。
    // 扣掉开局那 24 个半回合(那时确实无兵可调,闲置兵=0.0),
    // 每局仍有约 17.5 个中后期半回合是纯白扔。
    //
    // "高手此刻不动"的含义应该是**不出击**,而不是"连自己领地内的调兵也不做" ——
    // 在自己地里移动不消耗兵力(栈 A → (A-1)+1 = A),只花时间,
    // 而这个时间本来就是要扔掉的。所以聚兵是**真正免费**的。
    // PAUSE_GATHER=0 时退回 v51 行为(对照组)。
    // 【2026-07-30 实测回滚:默认改回 0】
    // v54 上线后 12 局真人实战 0 胜 11 负;控制对手星级后仍是
    //   >=25★: v51 7-6(53.8%) vs v54 0-10;>=30★: v51 6-6 vs v54 0-8
    // (若真实胜率 53.8%,连输 10 局概率 0.044%),局长中位 166 → 328。
    // 折返率两版相同(2.68% vs 2.97%),不是原地震荡,是打不完、被强真人磨死。
    // 机理推测:pause 信号是从高手数据学出来的(高手是刻意不动),
    // 用聚兵覆盖它 = 在高手明确选择"不动"的时刻偏离高手 ——
    // 离线对手不惩罚这种偏离,强真人会。
    // **三个离线裁判(siege/corpus/直接对位)全部给了正号,全部错了。**
    if (mm && mm.pause && P('PAUSE_GATHER', 0)) {
      const free = this.tryGather();
      if (free) return free;
      return null;
    }
    if (mm) return mm.pause ? null : mm; // 模型判断"此刻高手会不动"就真的不动
    return (
      this.tryExpand() ||
      this.tryHarass() ||
      this.tryGather()
  ...
  /**
   * 模仿学习走子(混合体的"日常行军"):用 469 局高手数据训的 MLP 排序器选步。
   * 规则仍握着生死权(斩首/防守/反蹲/攻城/波次/节奏都排在它之前),
   * 模型只接管"没有紧急事项时往哪走"。两道安全罩(沿用 v24 验证过的):
   *  - 兵临城下不掏将军(模型没有防守概念);
   *  - 禁止立即折返(排序器无记忆,会来回震荡——高手回头率只有 3.5%)。
   */
  tryModelMove() {
    if (process.env.NO_MODEL) return null;
    if (this.imitator === undefined) {
      try {
        const { ImitationStrategy } = require('./imitation6');
    ...
    if (!mv) return null;
    if (mv.pause) return mv;
    const gs = this.gs;
    // 罩1:当下真有威胁(敌方 >=6 兵在将军 5 格内)才不拿将军的兵当兵源。
    // 用"当下"而不是 homeExposed 记忆锁——后者会连续 30 个半回合冻结主力。
    if (mv.from === gs.myGeneral()) {
      const gen = gs.myGeneral();
      for (let t = 0; t < gs.size; t++) {
        if (gs.isEnemy(t) && gs.armies[t] >= 6 && gs.dist(t, gen) <= 5) return null;
      }
    }
    // 罩2:禁止立即折返
    if (this.lastModelMv && mv.from === this.lastModelMv.to && mv.to === this.lastModelMv.from) return null;
    this.lastModelMv = mv;
    return mv;
  }
  ...
  openingWave() {
    ...
    if (this.waveCount >= P('WAVE_MAX', 4)) return null; // 波次走完,交回常规逻辑
    const gen = gs.myGeneral();
    // 每波都用将军攒下的全部兵"一次性走完":第 1 波 turn24 自然有 13 兵;
    // 后面的波至少攒到 4 兵再出,避免两三兵的小碎波。
    const minArmy = this.waveCount === 0 ? 2 : P('WAVE_MIN2', 3);
    ...
    const ownPrefix = this.waveCount === 0 ? 0 : this.waveCount === 1 ? P('WAVE_PFX2', 2) : P('WAVE_PFX3', 6);
    ...
    if (gs.armies[gen] >= 8) this.waveCount = P('WAVE_MAX', 4);
    return null;
  }
  ...
  tryScout() {
    ...
    // 目标选择:敌将藏在**对手领地的后方**,不在地图边角。
    // 旧实现取"方向投影最大"的格且无上限,结果是径直越过对手冲进其身后的空地
    // (实战第二局主力被反复拉到 r16~r17,整局没找到敌将)。两条修正:
    //   (a) 投影封顶:不比"已见敌格的最深投影"再深出 SCOUT_CAP 倍;
    //   (b) 贴着敌方领地找雾:用到敌格的多源距离场惩罚远离对手领地的目标。
    let enemyMaxProj = 0;
    for (const t of enemy) {
      const p = (gs.row(t) - gr) * dr + (gs.col(t) - gc) * dc;
      if (p > enemyMaxProj) enemyMaxProj = p;
    }
    const projCap = enemyMaxProj * P('SCOUT_CAP', 1.05);
    // 【v50】不再"取最深且不超过封顶",改为**瞄准先验深度**。
    //
    // 封顶用"对手可见纵深"当参照,而这个代理量会被真人破坏:25 局真实排位对局
    // 重放审计显示,打真人时对手将军有 90.6% 的采样时点落在封顶之外(打 bot 只有 36.5%,
    // 且到 ht300 自动归零)—— bot 会一路推进所以纵深自然变深,真人不会。
    //
    // 1804 局语料实测双将曼哈顿距离中位 19.7;实测对手将军沿射线在 10~22.3 格处。
    // 所以直接瞄这个深度,既不会冲过头,也不依赖对手的行为规律。
    const TARGET_DEPTH = P('SCOUT_DEPTH', 20);
    // 【v51】按对手类型切换:平台用 "[Bot] " 前缀公开标记机器人,开局就拿得到用户名。
    //   机器人对手 → 用封顶(离线最优,25 局重放审计显示打 bot 时排除率仅 36.5%
    //                且到 ht300 自动归零 —— 因为 bot 会一路推进,可见纵深自然变深);
    //   真人对手   → 瞄准先验深度(打真人时封顶排除率高达 90.6%,是机制性封锁)。
    // 这不是取巧:用的是平台自己公开的标记,而且只影响我方的搜索策略。
    if (this._oppIsBot === undefined) {
      const u = gs.usernames;
      if (!u || !u.length) this._oppIsBot = false;
      else {
        this._oppIsBot = u.every((name, i) => i === gs.playerIndex || /^\[Bot\]/.test(String(name)));
      }
    }
    const AIM = P('SCOUT_AIM', this._oppIsBot ? 0 : 1);
    const dirLen = Math.hypot(dr, dc) || 1;
    const dfEnemy = bfsDistance(gs, enemy); // 各格到最近敌格的步数(-1 = 不可达)
    const NEAR_W = P('SCOUT_NEAR', 2);
    ...
    for (let t = 0; t < gs.size; t++) {
      ...
      const near = dfEnemy[t] === -1 ? 30 : dfEnemy[t];
      let score;
      if (AIM) {
        // 沿射线的实际格数 = 投影 / 方向向量长度
        const along = proj / dirLen;
        score = -Math.abs(along - TARGET_DEPTH) - NEAR_W * near;
      } else {
        if (proj > projCap) continue;
        score = proj - NEAR_W * near;
      }
  ...
  popValidQueued() {
    ...
    while (this.queue.length) {
      ...
      if (gs.isMine(mv.from) && gs.armies[mv.from] > 1 && gs.isPassable(mv.to)) {
        // 【v54】兵不够就别撞塔。
        //
        // marchCost 用 allowCity:true,路径可以**穿过**城市;而这里原来只校验
        // isPassable(城市是可通行的),于是"路过"直接变成"送死" —— 整摞兵撞上去清零。
        //
        // 107 局真人实战实测(city_audit.js):16 次/79 局,白扔 516 兵
        // (中位 26、均值 32.3,其中 7 次带 30+ 兵去撞守军中位 46 的塔)。
        // 频率不高但每次重伤,且**败局里的频率是胜局的近 4 倍**(0.35 vs 0.09 次/局)。
        //
        // 修在校验层而不是各个 try* 里,这样所有来源(strike/city/gather/模型走子)都兜得住。
        if (P('CITY_NOBOUNCE', 1) && gs.isCity(mv.to) && !gs.isMine(mv.to) &&
            gs.armies[mv.from] - 1 <= gs.armies[mv.to]) {
          this.queue = [];
  ...
  tryCaptureCity() {
    const gs = this.gs;
    const src = this.biggestArmyTile([], true);   // 【v57】自己可以用预留堆
    if (src === -1) return null;
    ...
    // 中立塔:家没被发现是硬条件。敌将已知时不是全停——产能落后对手就边骚扰边补塔
    // (产能落后硬冲也攒不过他);产能领先才全力攒兵杀将。敌将未知照旧滚经济。
    const canNeutralCity = oppJustBought || (!this.homeSeen &&
      (enemyGenKnown
        ? incomeBehind
        : ((!top || Math.abs(myTotal - top.total) < 10) || gs.turn >= 100)));

    // 防守性判据(实战教训):打下来守不住的塔是纯送 —— 投进 40+ 兵,驻军归零,
    // 对手一个兵团就能连塔带投资一起收走。中立塔必须在"我这一侧"才值得投。
    // 只用可见敌格算距离:迷雾里的威胁本来就看不见,所以靠"离谁近"这个
    // 几何判据兜底,而不是指望侦测到具体兵团。
    const enemyTilesForCity = [];
    for (let t = 0; t < gs.size; t++) if (gs.isEnemy(t)) enemyTilesForCity.push(t);
    const CITY_SIDE = P('CITY_SIDE', 0); // 中立塔:允许比对手近多少格的松弛量
    // 【v54】敌方塔原来**完全没有**守得住判据("另一套账"直接 return true)。
    //
    // 107 局真人实战审计(city_audit.js,79 局打完的)显示这是纯亏:
    //   中立塔  32 次,被夺回 46.9%,但持有中位 **128 回合**(丢也是残局丢,健康)
    //   敌方塔  84 次,被夺回 51.2%,持有中位 **5 回合** ——
    //           43 次打下来、把中位 23 兵留在那儿,五个回合后连塔带兵送给对手。
    //
    // 这里给敌方塔一个**独立且更宽松**的松弛量:塔到我将军的距离不能比
    // 到最近敌格的距离远出 CITY_SIDE_E 格。默认 99 = 完全等价于 v51(对照组)。
    const CITY_SIDE_E = P('CITY_SIDE_E', 99);
    const cityDefensible = (c) => {
      if (!enemyTilesForCity.length) return true; // 还没接触,无所谓侧
      const gen = gs.myGeneral();
      let dEnemy = Infinity;
      for (const t of enemyTilesForCity) {
        const dd = gs.dist(c, t);
        if (dd < dEnemy) dEnemy = dd;
      }
      const slack = gs.isEnemy(c) ? CITY_SIDE_E : CITY_SIDE;
      return gs.dist(c, gen) <= dEnemy + slack;
    };

    const eligible = (c) => (gs.isEnemy(c)
      ? (canEnemyCity && cityDefensible(c))
      : (canNeutralCity && cityDefensible(c)));
    ...
    for (const c of gs.knownCities) {
      if (gs.isMine(c) || !gs.isVisible(c) || !eligible(c)) continue;
      // 【v57 修 bug】原来这里拿 dijkstra 的**加权代价**跟"6"比,而 marchCost
      // 对中立城会加 armies*0.3(40 兵的塔光最后一步就 1+12=13)——
      // 所以 `dist[c] > 6` 对任何真实的塔恒成立,**这个攒兵分支是死代码,从没运行过**
      // (CITY_DBG 实测 20 局触发 0 次)。这正好解释了城市经济为何只有真人的一半:
      // "差一点吃不下就先攒兵"这条路径根本不存在。
      // CITY_STEP=1 改用步数口径(gs.dist);默认 0 保持原行为,作为干净对照组。
      if (dist[c] === Infinity) continue;
      if (P('CITY_STEP', 0) ? gs.dist(src, c) > P('CITY_NEAR', 6) : dist[c] > 6) continue;
      if (gs.armies[src] > gs.armies[c] * 0.5) {
        // 【v57】攒到 CITY_LOCK 比例后把这一堆锁住,别让 raid/harass/scout 抽走。
        // 附带超时(CITY_LOCK_TTL 个半回合没吃下就解除),避免无限占用主力。
        const need = gs.armies[c] + gs.dist(src, c) + 2;
        const LOCK = P('CITY_LOCK', 0);
        if (process.env.CITY_DBG) { global.__cityB2 = (global.__cityB2||0)+1; global.__cityRatio = (global.__cityRatio||[]); global.__cityRatio.push(gs.armies[src]/Math.max(1,need)); }
        if (LOCK && gs.armies[src] >= need * LOCK) {
          if (process.env.CITY_DBG) global.__cityLock = (global.__cityLock||0)+1;
          if (this._cityReserve !== src) { this._cityReserve = src; this._cityReserveSince = gs.turn; }
          else if (gs.turn - this._cityReserveSince > P('CITY_LOCK_TTL', 60)) this._cityReserve = -1;
        }
  ...
  /**
   * 【v57】allowReserve:是否允许选用"打塔预留堆"。
   *
   * city_gather_audit.js 实测:打塔攒兵的 ratio(最大兵堆/吃塔所需)
   * **每局峰值中位 0.96** —— 攒到差 4% 就停了,53% 的局峰值从没跨过 1.0,
   * 14/30 局卡在 0.5~1.0。不是兵不够,是**临门一脚被别的行为抽走**
   * (记忆里那条"兵刚并进主力,立刻被 raid/harass/scout 按 biggestArmyTile 取走")。
   *
   * 所以只在"塔近 + 已攒到 CITY_LOCK 比例"时把那一堆锁住,其他调用方看不到它。
   * 范围刻意做窄:不是之前失败五次的通用"保护主力",只保护一次具体的采购。
   */
  biggestArmyTile(exclude, allowReserve) {
    const gs = this.gs;
    const ex = new Set(exclude);
    if (!allowReserve && this._cityReserve !== undefined && this._cityReserve >= 0) {
      const rt = this._cityReserve;
      if (gs.isMine(rt) && gs.armies[rt] > 1) ex.add(rt);
      else this._cityReserve = -1;   // 堆已经没了/丢了,解除预留
    }
    const gen = gs.myGeneral();
    let best = -1, bestA = 1;
    for (let t = 0; t < gs.size; t++) {
      ...
      // 【v54】倾向于不从家门口出击。
      //
      // 79 局真人实战(leak_audit.js)测出:"从将军 3 格内发起的走子占比" 与
      // "我方将军被对手发现的回合" 强负相关 **r = -0.665**(n=42)——
      // 早被发现的局这个比例 43.4%,晚被发现的 28.3%,从未被发现的 29.0%。
      // 而"聚兵回家"那一半没有效应(r = +0.159),两者必须分开看。
      // 这很重要,因为"我方将军被发现"是 **100% 败局**的共同点。
      //
      // 原有保护只覆盖两种情况:homeExposed(敌兵已贴到 4 格内 —— 那是防守触发器,
      // 对隐蔽已经太晚)、以及 turn>100 留一半。t100 之前将军整摞兵是敞开可用的,
      // 而被发现的中位回合是 t116。
      //
      // 这里对"家附近"的兵源打折(惩罚而非禁止:没有替代兵源时仍会选它),
      // HOME_PEN=1 时完全等价于 v51。
      const HOME_PEN = P('HOME_PEN', 1);
      const HOME_R = P('HOME_R', 2);
      if (HOME_PEN !== 1 && t !== gen && gs.dist(t, gen) <= HOME_R) a *= HOME_PEN;
      if (t === gen) {
        // 兵临城下(敌兵近期出现在将军 4 格内)时,将军的兵一个都不出门——
        // 之前的漏洞:防守把兵聚回家,威胁一蹲家不动,侦察/打塔/突袭
        // 就把这坨"全图最大兵团"当兵源调出门,等于开门揖盗。
        if (this.homeExposed) continue;
        // 平时:中后期留一半,早期只留 1(开局要用将军兵扩张)
