...
class Strategy {
  ...
  openingWave() {
    ...
    const path = this.waveWalk(gen, ownPrefix);
  ...
  tryCaptureCity() {
    ...
    // 落后超过 30 兵就别恋战攻塔(经济劣势时塔会拖死你);30 以内碰到吃得动的就吃。
    const top = gs.enemyScores()[0];
    if (top && gs.myScore().total < top.total - 30) return null;
    ...
    for (const c of gs.knownCities) {
      if (gs.isMine(c) || !gs.isVisible(c)) continue;
  ...
  /** 把(除目标外)最大的兵团往 target 挪:专走兵多的己方地(顺路收编),绕开 1 兵地 */
  gatherToward(target) {
    const gs = this.gs;
    const src = this.biggestArmyTile([target]);
    if (src === -1 || src === target) return null;
    const { prev, dist } = dijkstra(gs, src, gatherCost(gs));
