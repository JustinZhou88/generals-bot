...
class Strategy {
  ...
  nextMove() {
    ...
    const consolidate = gs.turn >= 300 ? this.tryGather() : null;
    return (
      this.tryStrike() ||
      this.tryCounterCamp() ||
  ...
  raidWalk(src) {
    ...
    while (strength > 1) {
      ...
      for (const n of gs.neighbors(cur)) {
        ...
        const score = (enemyTile ? 1000 : 0) - dGoal * 3 - cost;
  ...
  tryCaptureCity(immediateOnly = false) {
    ...
    const cityTarget = realT < 55 ? 0 : 1 + Math.max(0, Math.floor((realT - 100) / 50));
