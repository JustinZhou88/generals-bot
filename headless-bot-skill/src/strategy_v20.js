...
class Strategy {
  ...
  nextMove() {
    ...
    // 发育段(phase<30):先算集兵提前量——主力离前线多远,就提前多少半回合开始收兵,
    // 让偷袭窗口一开兵团已在前线蓄势(这就是"自己计算恰当的集兵时间")。
    const preGather = (() => {
      const lead = this.gatherLead();
      if (lead !== null && phase >= 30 - lead) return this.tryGather();
      return null;
    })();
    return (
      this.tryStrike() ||
      ...
      this.tryHarass() ||      // 更有攻击性:能占便宜的相邻敌格随时咬
      this.tryScout() ||
      this.tryExpand() ||
      this.tryGather()
    );
  ...
  gatherLead() {
    ...
    return Math.min(16, gs.dist(src, front) + 4);
  }
  ...
  tryCaptureCity(immediateOnly = false) {
    ...
    // 中立塔:硬门槛从"家被看过"放宽为"家正被威胁"(homeExposed)——
    // 对手摸到过家但眼下没在压家,照样可以投塔滚经济,否则实战里永远不打塔。
    const canNeutralCity = oppJustBought || (!this.homeExposed &&
      (enemyGenKnown
        ? incomeBehind
