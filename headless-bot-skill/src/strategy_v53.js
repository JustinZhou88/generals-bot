...
class Strategy {
  ...
  nextMove() {
    ...
    // 【v53】防守抢占斩首时,把斩首计划**存起来**而不是丢掉。
    //
    // strike_audit.js 实测(60 局 88 次斩首):走完计划的斩首率 80.9%、净地 +13.2;
    // 半路夭折的斩首率暴跌到 24.4%、净地 -28.9。夭折的 41 次里:
    //   路径失效 21 次(已走 12.5/16.1 步,净地 -7.3)
    //   被 defend 抢占 20 次(只走了 6.5/21.4 步,净地 **-51.5**)
    // 机制原因:planPath 会清空 queue,而系统里没有任何"续接"机制 ——
    // 一次防守动作就把 40 兵永久扔在敌区不管了。
    //
    // 注意:这份数据是相关性。defend 触发本身意味着我们正在挨打,
    // -51.5 里有多少来自"挨打"、多少来自"放弃斩首",只有做实验才分得开。
    // 快照必须在 checkDefense **之前**取:它内部会调 planPath('defend'),
    // 那一步已经把 queue 清空、queuePurpose 改成 'defend' 了 ——
    // 在它返回之后再读,永远读不到斩首队列(初版就是这么写的,结果改动完全无效,
    // corpus_arena 跑出与对照组逐位相同的 64.13%)。
    const preStrike = (this.queuePurpose === 'strike' && this.queue.length)
      ? this.queue.slice() : null;
    const defense = this.checkDefense();
    if (defense) {
      if (P('STRIKE_RESUME', 0) && preStrike) {
        this._suspended = { queue: preStrike, purpose: 'strike' };
      }
      return defense;
    }
    ...
    // 防守打完了、手上没别的计划 —— 把之前被打断的斩首接回来。
    // 路径可能已经失效(兵没跟上/地丢了),但 popValidQueued 会逐步校验并自动放弃,
    // 所以恢复一个过期计划是安全的。
    if (P('STRIKE_RESUME', 0) && this._suspended) {
      this.queue = this._suspended.queue;
      this.queuePurpose = this._suspended.purpose;
      this._suspended = null;
      const resumed = this.popValidQueued();
  ...
  tryStrike(killOnly = false) {
    ...
    for (const [p, genTile] of gs.knownGenerals) {
      ...
      if (gs.armies[src] > need) {
        // 【v53】路径太长的斩首更容易被防守抢占:被 defend 截断的那批计划步数
        // 均值 21.4,而整体均值只有 16.6 —— 路越长,暴露窗口越大。
        // STRIKE_MAXLEN 限制发动距离;0 = 不限(退回 v51 行为)。
        const maxLen = P('STRIKE_MAXLEN', 0);
        if (maxLen && path.length - 1 > maxLen) {
          if (killOnly) continue;
          const g = this.gatherToward(src);
          if (g) return g;
          continue;
        }
